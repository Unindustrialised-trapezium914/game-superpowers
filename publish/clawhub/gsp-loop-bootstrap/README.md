# gsp-loop-bootstrap

Part of the **Game Superpowers** skill system.

## Summary

Use when establishing core game runtime structure such as state flow, control, camera, and UI ownership.

## Recommended usage

- Use `gsp-loop-bootstrap` for this game project and keep the output evidence-based.
- Apply `gsp-loop-bootstrap` to improve or assess an existing browser game in a focused way.
- Use `gsp-loop-bootstrap` as part of a larger Game Superpowers workflow when the request clearly matches game loop bootstrap.

## Companion skills

- `gsp-orchestrator`

## Source of truth

This publish bundle is derived from the Game Superpowers source repository.
Canonical authoring lives in:

```text
skills/gsp-loop-bootstrap/SKILL.md
```

## Notes

- This bundle is published for OpenClaw / ClawHub distribution.
- The Game Superpowers repository also supports Claude Code and Codex distributions.
- If a workflow references companion skills, install those companion skills as separate bundles.
