# gsp-compare-backends

Part of the **Game Superpowers** skill system.

## Summary

Use when backend uncertainty remains and only a narrow two- or three-option compare is needed.

## Recommended usage

- Use `gsp-compare-backends` for this game project and keep the output evidence-based.
- Apply `gsp-compare-backends` to improve or assess an existing browser game in a focused way.
- Use `gsp-compare-backends` as part of a larger Game Superpowers workflow when the request clearly matches game compare backends.

## Companion skills

- `gsp-orchestrator`

## Source of truth

This publish bundle is derived from the Game Superpowers source repository.
Canonical authoring lives in:

```text
skills/gsp-compare-backends/SKILL.md
```

## Notes

- This bundle is published for OpenClaw / ClawHub distribution.
- The Game Superpowers repository also supports Claude Code and Codex distributions.
- If a workflow references companion skills, install those companion skills as separate bundles.
