# gsp-procedural-art-direction

Part of the **Game Superpowers** skill system.

## Summary

Use when procedural visual identity, shaders, particles, SVG systems, or UI recipes are part of the game deliverable.

## Recommended usage

- Use `gsp-procedural-art-direction` for this game project and keep the output evidence-based.
- Apply `gsp-procedural-art-direction` to improve or assess an existing browser game in a focused way.
- Use `gsp-procedural-art-direction` as part of a larger Game Superpowers workflow when the request clearly matches game procedural art direction.

## Companion skills

- `gsp-orchestrator`

## Source of truth

This publish bundle is derived from the Game Superpowers source repository.
Canonical authoring lives in:

```text
skills/gsp-procedural-art-direction/SKILL.md
```

## Notes

- This bundle is published for OpenClaw / ClawHub distribution.
- The Game Superpowers repository also supports Claude Code and Codex distributions.
- If a workflow references companion skills, install those companion skills as separate bundles.
