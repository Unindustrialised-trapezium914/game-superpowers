# gsp-implementation-plan

Part of the **Game Superpowers** skill system.

## Summary

Use when turning an approved game brief into verifiable implementation tasks matched to the chosen build mode.

## Recommended usage

- Use `gsp-implementation-plan` for this game project and keep the output evidence-based.
- Apply `gsp-implementation-plan` to improve or assess an existing browser game in a focused way.
- Use `gsp-implementation-plan` as part of a larger Game Superpowers workflow when the request clearly matches game implementation plan.

## Companion skills

- `gsp-orchestrator`

## Source of truth

This publish bundle is derived from the Game Superpowers source repository.
Canonical authoring lives in:

```text
skills/gsp-implementation-plan/SKILL.md
```

## Notes

- This bundle is published for OpenClaw / ClawHub distribution.
- The Game Superpowers repository also supports Claude Code and Codex distributions.
- If a workflow references companion skills, install those companion skills as separate bundles.
