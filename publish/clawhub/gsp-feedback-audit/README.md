# gsp-feedback-audit

Part of the **Game Superpowers** skill system.

## Summary

Use when auditing responsiveness, rewards, failures, danger telegraphing, or state-transition feedback in a game.

## Recommended usage

- Use `gsp-feedback-audit` for this game project and keep the output evidence-based.
- Apply `gsp-feedback-audit` to improve or assess an existing browser game in a focused way.
- Use `gsp-feedback-audit` as part of a larger Game Superpowers workflow when the request clearly matches game feedback audit.

## Companion skills

- `gsp-orchestrator`

## Source of truth

This publish bundle is derived from the Game Superpowers source repository.
Canonical authoring lives in:

```text
skills/gsp-feedback-audit/SKILL.md
```

## Notes

- This bundle is published for OpenClaw / ClawHub distribution.
- The Game Superpowers repository also supports Claude Code and Codex distributions.
- If a workflow references companion skills, install those companion skills as separate bundles.
