...
- Use `gsp-spec-driven-planning` when the user explicitly wants `requirements/spec/design/tasks` artifacts in the repo.
- Apply `gsp-spec-driven-planning` when a game feature needs durable change records that survive across sessions.
- Use `gsp-spec-driven-planning` on top of the normal Build, Repair, Audit, or Polish route instead of replacing those tracks.
...
```text
skills/gsp-spec-driven-planning/SKILL.md
```
...
- Install companion skills separately when a routed workflow depends on them.
