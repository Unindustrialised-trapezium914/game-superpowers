# game-live-patch

Part of the **Game Superpowers** skill system.

## Summary

Use when the repo is shipped or live-risky. This skill narrows changes, documents blast radius and rollback, and prioritizes safety over broad cleanup.

## Recommended usage

- Use `game-live-patch` for this game project and keep the output evidence-based.
- Apply `game-live-patch` to improve or assess an existing browser game in a focused way.
- Use `game-live-patch` as part of a larger Game Superpowers workflow when the request clearly matches game live patch.

## Companion skills

- None

## Source of truth

This publish bundle is derived from the Game Superpowers source repository.
Canonical authoring lives in:

```text
skills/game-live-patch/SKILL.md
```

## Notes

- This bundle is published for OpenClaw / ClawHub distribution.
- The Game Superpowers repository also supports Claude Code and Codex distributions.
- If a workflow references companion skills, install those companion skills as separate bundles.
