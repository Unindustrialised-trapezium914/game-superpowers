...
- Use `gsp-subagent-build-loop` for this game project and keep the output evidence-based.
- Apply `gsp-subagent-build-loop` to improve or assess an existing browser game in a focused way.
- Use `gsp-subagent-build-loop` as part of a larger Game Superpowers workflow when the request clearly matches game subagent build loop.
...
```text
skills/gsp-subagent-build-loop/SKILL.md
```
